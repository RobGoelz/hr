...
def test_users_add(mocker):
    ...
    utilize `useradd` to create a user with the
    password and groups.
    """
...
def test_users_update(mocker):
    ...
    utilize `usermod` to set the groups and password
    for the user.
    """
    mocker.patch('subprocess.call'),
...
def test_users_sync(mocker):
    """
    Given a list of user dictionaries, `users.sync(...)`
    should create missing users, remove extra non-system
    users, and update existing users. A list of existing
    usernames can be passed in or default users will be used.
    """
